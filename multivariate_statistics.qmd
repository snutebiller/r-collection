...
$$
Since $X^W$ can be inferred from the covariance matrix of $X$, only $\pmb{M}^T$ has to be estimated. This estimation is based on the non-Gaussianity assumption mentioned previously, and allows to uniquely identify the latent sources $S$ up to permutations, sign changes, and scaling. If the sources are gaussian distributed, the latent sources can only be identified up to rotations (as $\pmb{M}^T$ is not identifiable).
...
## Example
ICA can be used to disentangle merged images. Images were cropped to have the same dimensions and are stored in the form of matrices, in which the row $i$ and column $j$ index represent the pixel $(i,j)$, and the value of the matrix at $i,j$ is the grey value at that pixel. For the purpose of plotting with `ggplot2`, the data was transformed into a `data.frame` with columns that represent the row and column indexes, and the grey values.
```{r}
library(imager)
...
Artificial images were created by multiplying the image-matrices with a constant, adding the image matrices, and adding normal-distributed noise with a standard devation of 0.1.
```{r}
#| tbl-cap: Weights used to create mixed images

ica_weights <- list(c(0.2, 0.8), c(0.7, 0.3), c(0.4, 0.1), c(0.1, 0.3))
kable(as.data.frame(ica_weights, row.names = c('Picasso', 'Jackson')),
      align = 'c',
      row.names = TRUE,
      col.names = 1:4,
      table.attr = 'data-quarto-disable-processing="true"') |>
...
set.seed(78)
tot_pix <- nrow(p1)
ica_mixed_images <- lapply(1:length(ica_weights), function(x) {
  p1_temp <- p1
  p2_temp <- p2
  new_df <- p1
  new_df$z <- ica_weights[[x]][1] * p1_temp$z + ica_weights[[x]][2] * p2_temp$z + rnorm(tot_pix, sd = 0.1)
  new_df$plot <- x
  new_df
  })

ica_mixed_im_df <- do.call(rbind, ica_mixed_images)

ggplot(ica_mixed_im_df, aes(x = x, y = y, fill = z)) +
  geom_tile() +
  facet_wrap(~plot) +
  scale_fill_gradient(low = 'black', high = 'white')  +
  theme(legend.position = 'none')
...
Independent component analysis can be performed in `R` with the `fastICA` library. The data matrix in this example contains the pixel _values_ represented in rows, and the different images in different columns. There is no information on the position of the pixel in the input image, except that the position is the same for all the images.
```{r}
#| tbl-cap: Organization of data for analysis by `fastICA`. Rows contain the grayscale value for the same pixel, while represent different images. 

ica_dfmixed <- ica_mixed_images |>
  lapply(function(x) x$z) |>
    data.frame()
colnames(ica_dfmixed) <- paste('imag', 1:4, sep = '')

kable(as.data.frame(ica_dfmixed[1:5,]),
      table.attr = 'data-quarto-disable-processing="true"',
      align = 'c') |>
  kable_styling(full_width=FALSE)
```
...
The estimated images can be extracted from the `$S` column of the `fastICA` results list. Since ICA can identify independent components only up to sign changes, the sign may have to be changed.
```{r}
ica_res <- lapply(1:ncol(ica$S), function(x){
  res <- imMtoDF(matrix(ica$S[,x], nrow = ica_p_dims[1], ncol = ica_p_dims[2]))
  res$z <- ica_norm_grey(res$z)
  res
})
...
ggplot(ica_res_m, aes(x = x, y = y, fill = -1*z)) +
  geom_tile() +
  facet_wrap(~plot) +
  scale_fill_gradient(low = 'black', high = 'white')  +
  theme(legend.position = 'none')
